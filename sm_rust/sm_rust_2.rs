...
fn main() -> Result<(), Box<dyn core::error::Error>> {
    ...
    println!("[sm_rust_2] sm_rust_2 is ready");
    ...
    while node.wait(CYCLE_TIME).is_ok() {
        ...
        println!(
            "[sm_rust_2] Published → temp: {:.2}, hum: {}, current: {:.2}, timestamp: {}",
            ...
            .send()
        {
            Ok(response) => {
                if response.status().is_success() {
                    println!("[sm_rust_2] Sent data to REST API successfully!");
                } else {
                    println!("[sm_rust_2] REST API error: {}", response.status());
                }
            }
            Err(err) => println!("[sm_rust_2] Failed to send data: {}", err),
        }
        ...
        if let Some(sample) = subscriber_humidity.receive()? {
            println!("[sm_rust_2] Received hum: {}", *sample);
        }
        if let Some(sample) = subscriber_current.receive()? {
            println!("[sm_rust_2] Received current: {:.2}", *sample);
        }
    }
    Ok(())
}
