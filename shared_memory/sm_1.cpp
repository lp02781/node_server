...
int main() {
    ...
    iox::capro::ServiceDescription temperature_service("sensor_data", "sm_1", "temperature");
    iox::capro::ServiceDescription humidity_service("sensor_data", "sm_1", "humidity");
    iox::capro::ServiceDescription current_service("sensor_data", "sm_1", "current");
    ...
    std::cout << "[sm_1] sm_1 is ready" << std::endl;
    ...
    while (true) {
        ...
        std::cout << "[sm_1] Publishing temp: " << temperature 
                  << " hum: " << humidity 
                  << " current: " << current << std::endl;

        if (auto sample = sub_temperature.take()) {
            std::cout << "[sm_1] Received temp: " << **sample << std::endl;
        }
        if (auto sample = sub_humidity.take()) {
            std::cout << "[sm_1] Received hum: " << **sample << std::endl;
        }
        if (auto sample = sub_current.take()) {
            std::cout << "[sm_1] Received current: " << **sample << std::endl;
        }
